...
public class reservationCont {
  ...
  @DeleteMapping("/{markerId}/reservations")
  public ResponseEntity<List<reservations>> deleteAllreservationssOfmarker(@PathVariable(value = "markerId") Long markerId) {
    if (!markerRepo.existsById(markerId)) {
      throw new UserNotFoundException("Not found marker with id = " + markerId);
    }
    reservationRepo.deleteByMarkerId(markerId);
    return new ResponseEntity<>(HttpStatus.NO_CONTENT);
  }
}
